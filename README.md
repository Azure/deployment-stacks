...
To go through a deploymentStacks tutorial, select [tutorial](./TUTORIAL.md).
...
* A deploymentStack currently does not manage resourceGroups, subscriptionAliases, or managementGroups that are created by the deploymentStack.
...
You can use any ARM template to create a deploymentStack. The following template is an example. This template creates two storage accounts.
...
It takes a few moments to create a deploymentStack.  Once completed, you can run the following cmdlet to get the deploymentStack:
...
Notice the `UpdateBehavior` property in the output. `New-AzResourceGroupDeploymentStack` can also be used to update an existing deploymentStack, same as [`Set-AzResourceGroupDeploymentStack`](#add-remove-resources). When it is used to update an existing deploymentStack, the `-UpdateBehavior` switch is used.  The default value for the switch is `detachResources`. The value can also be set to `purgeResources`. For more information, see [Add/remove resources](#add-remove-resources).
...
Deploy the deploymentStack with the following cmdlets:
...
It takes a few moments to create a deploymentStack.  Once completed, use the following cmdlet to get the deploymentStack:
...
Modify the original template to remove an existing resource or add a new resource, and then use `Set-AzResourceGroupDeploymentStack` or `Set-AzSubscriptionDeploymentStack` to update the deploymentStack. When you remove a resource from a deploymentStack, you have two options with the `UpdateBehavior` switch:
...
After updating the deploymentStack, use `Get-AzResourceGroupDeploymentStack` to list the resources in the deploymentStack.
...
The `Location` parameter specifies where the deploymentStack is saved.

After updating the deploymentStack, use `Get-AzSubscriptionDeploymentStack` to list the resources in the deploymentStack.

## Use snapshots

Snapshots provide a way to view the history of updates to the deploymentStack. A snapshot is read-only and is created whenever any template resource is successfully deployed.  Note that not all resources need to be successfully deployed for a snapshot to be created. Snapshots are primarily used for viewing history for diagnostics or troubleshooting. In the future, a snapshot can be applied to roll back a deploymentStack to a previous state if there is an error.

Since it contains more information about the state of the deploymentStack, the latest snapshot of a deploymentStack cannot be deleted.
...
To remove a snapshot from a deploymentStack:
...
## Delete a deploymentStack

Removing a deploymentStack also removes the all snapshots. All managed resources will be detached.
...
To use a remote template, use the `-TemplateUri` switch; to use a remote parameter file, use the `-TemplateParameterUri`:
